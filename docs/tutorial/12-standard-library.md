...
Each condition is tested in turn and the first true one wins; conditions after it are never evaluated, and neither are the values of branches that lose. `when` builds a `maybe`, `elseWhen` fills it in if it is still `none`, and `default` unwraps it. There is no special syntax here — they are ordinary functions, which is what lets every part of the chain have a type.
