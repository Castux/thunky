...
the list is long enough — `--types` reports where those beliefs are, see
[7. Type Analysis](implementation/7.Type%20Analysis.md).
...
Prefer `headOr 0 x` to `if (isEmpty x) 0 (head x)`. They say the same thing, but
only the first says it in a form the analysis can check.
